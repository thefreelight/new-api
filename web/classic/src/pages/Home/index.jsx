...
const Home = () => {
  ...
  const brandName = statusState?.status?.system_name || getSystemName();
  const displayBrandName =
    brandName === 'New API' || brandName === 'NavtoAI API'
      ? 'NavtoAI'
      : brandName;
  const productName =
    displayBrandName === 'NavtoAI' ? 'NavtoAI API' : displayBrandName;
  const rawServerAddress =
    statusState?.status?.server_address || `${window.location.origin}`;
  const serverAddress = rawServerAddress.replace(/\/$/, '');
  ...
  const terminalEndpoint = useMemo(
        ...
        <div className='site-container hero-grid'>
          <div className='hero-copy'>
            ...
            <div className='hero-actions'>
              ...
              <a className='ghost-button' href='#quick-start'>
                查看接入方式
              </a>
            </div>
          </div>
      ...
      <section className='site-section quick-section' id='quick-start'>
        <div className='site-container two-column'>
          <div className='section-copy'>
            ...
            <h2>几分钟接入常用 AI 工具</h2>
            <p>
              把 Base URL 和 API Key 指向 {productName}
              ，即可让常见开发工具和兼容 SDK
            ...
            <div className='hero-actions compact'>
              <a className='orange-button' href='#pricing'>
                查看价格说明
              </a>
...
export OPENAI_API_KEY="navto-demo-key"

curl ${terminalEndpoint} \\
        ...
        <div className='site-container'>
          <div className='section-heading split'>
            <div>
              ...
              <h2>{productName} 专注 AI 网关与企业级模型接入</h2>
              <p>
                打造统一的大模型中转与管理平台，为个人开发者、团队和企业客户提供模型接入、Token
                管理、渠道调度、用量监控和客户支持等一体化服务。
              </p>
            </div>
            <a className='ghost-button' href='#quick-start'>
              查看接入文档
          ...
          <div className='operations-board'>
            <article className='ops-card ops-card-large'>
              <div>
                <span className='ops-label'>控制台现场</span>
                <h3>稳定维护 AI 网关与模型渠道</h3>
                <p>渠道、模型、Token、日志和用量排查集中在一个运营视图里。</p>
              </div>
              <div className='ops-console-shot'>
                <img src='/cover-4.webp' alt='NavtoAI 模型网络视觉' />
              </div>
            </article>
            <article className='ops-card'>
              <LineIcon Icon={Database} />
              <span className='ops-label'>配额管理</span>
              <h3>余额、扣费和用量趋势</h3>
            </article>
            <article className='ops-card'>
              <LineIcon Icon={KeyRound} />
              <span className='ops-label'>客户交付</span>
              <h3>按项目拆分 Key 与权限</h3>
            </article>
            <article className='ops-card ops-card-wide'>
              <LineIcon Icon={Globe2} />
              <span className='ops-label'>渠道调度</span>
              <h3>围绕接入流程、路由策略和客户支持协作</h3>
            </article>
          </div>
        </div>
      </section>
      ...
      <section className='site-section prompt-section'>
        <div className='site-container prompt-layout'>
          <div className='section-copy'>
            ...
            <h2>从提示词案例开始生成图片</h2>
            <p>
              提示词库收录可复用案例，支持按分类搜索、复制 prompt，并可直接带入
              Playground 试图。
            </p>
            <div className='hero-actions compact'>
              <Link className='orange-button' to='/console/playground'>
                打开生图试验场
              </Link>
              <a className='ghost-button' href='#models'>
                浏览能力
              </a>
            </div>
          </div>
      ...
      <section className='site-section affiliate-section' id='agent'>
        <div className='site-container affiliate-layout'>
          <div className='section-copy'>
            ...
            <p>
              {productName}{' '}
              为代理提供项目介绍、产品培训、推广素材和客户承接支持，适合愿意通过内容和私域长期积累客户的人。
            </p>
            <div className='hero-actions compact'>
      ...
      <section className='site-section pricing-preview' id='pricing'>
        <div className='site-container'>
          <div className='section-heading split'>
            ...
            <Link className='ghost-button' to={primaryLink}>
              查看控制台
            </Link>
          </div>
